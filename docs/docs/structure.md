...
`make inputs-pull` can be used to pull data from the configured S3 bucket.

### `outputs/`

Store any outputs - e.g. output datasets, models, reports, figures - that need to be locally persisted here.

You may wish to version control some outputs such as reports.

### Sub-folder structure

Depending on your use-case you may opt for different sub-folder structures.

If your cookiecutter has a relatively narrow scope, you could organise by type of artifact first: e.g. `outputs/data/job_listings.parquet`.
If your cookiecutter contains a lot of different types of analysis, you could organise by theme first: e.g. `outputs/heat_pumps/adoption_stats.parquet`.
...
For interim datasets or models, you could use a `.cache/` folder to avoid confusion between interim and final data/models.
...
│   └── README.md                    |    Documentation on the inputs, e.g. their provenance
├── jupytext.toml                    |  JUPYTEXT CONFIGURATION
├── LICENSE                          |
├── outputs                          |  OUTPUTS PRODUCED FROM THE PROJECT
│   └── README.md                    |    Documentation about what outputs are generated by what
